...
def format_styled_mobile_df(df, sort_cols, main_col, sub_col, is_buy=True):
    """手機版表格格式化與上色工具"""
    # ★ 這裡把 head(50) 改成 head(100)
    df_sorted = df.sort_values(by=sort_cols, ascending=[False, False, False]).head(100)
...
# ★ 這裡把標題的 50 名都改成 100 名
with tab1:
    st.caption("外資買超前 100 名 (單位: 天數)")
